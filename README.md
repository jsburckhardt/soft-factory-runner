...
The current local npm package release is **0.2.1-beta.2**, a backward-compatible prerelease PATCH correction for explicit cleanup of an exactly owned remain-on-exit dead pane. This repository does not claim registry publication and the CLI has no `--version` command. Build and pack the checked-out release, then upgrade an existing 0.1.3 prefix or reinstall the same tarball:
