...
### Current package release and 0.2.1-beta.1-to-0.2.1-beta.2 upgrade

The current local npm package release is **0.2.1-beta.2**, a backward-compatible prerelease PATCH correction only for explicit cleanup of an exactly owned remain-on-exit dead pane. Prerelease history is intentionally separate: beta.0 prevents Doctor collapse when an unrelated/default tmux server is absent; beta.1 recognizes only an unchanged stale socket with exact bounded no-server evidence; beta.2 adds only guarded dead-pane cleanup. This repository does not claim registry publication and the CLI has no `--version` command. Build and pack the checked-out release, then upgrade an existing 0.1.3 prefix or reinstall the same tarball:

```text
just build
mkdir -p /tmp/soft-factory-runner-0.2.1-beta.2
npm pack --json --pack-destination /tmp/soft-factory-runner-0.2.1-beta.2
PREFIX="${SOFT_FACTORY_PREFIX:-$HOME/.local/soft-factory-runner}"
npm install --ignore-scripts --no-audit --no-fund --omit=dev --prefix "$PREFIX" /tmp/soft-factory-runner-0.2.1-beta.2/soft-factory-runner-0.2.1-beta.2.tgz
node -p "require('$PREFIX/node_modules/soft-factory-runner/package.json').version"
```

The metadata command must print exactly `0.2.1-beta.2`. For a clean reinstall, run `npm uninstall --prefix "$PREFIX" soft-factory-runner` before the same local-tarball install. From each target repository, reconverge package-coupled official assets and confirm the generated manifest:
...
The manifest command must also print exactly `0.2.1-beta.2`.
...
The package-local source `assets/official/soft-factory.agent.md` installs byte-for-byte at `.github/agents/soft-factory.agent.md`; strict schema-v1 ownership remains at `.agents/manifest.json` with package version 0.2.1-beta.2, Runner protocol 1, destination, and SHA-256. The npm allowlist names only that source, so assessor, skill, sibling, and local comparison files are not published. Removed assessor and skill selectors return `CLI_INVALID`.
...
Every product run names one explicit positive issue number. Runner never queries for, queues, ranks, or selects a next issue. `run` creates new state only; existing state returns `RUN_EXISTS` and must be inspected with reconciliation or control commands. Human and JSON status, reconciliation, and control output derive from one categorical public view. They agree on persisted state, activity, decision/outcome, eligibility, safe actions, cleanup-category progress, refusal, remediation, and exit meaning. Neither form serializes the reconciliation report, raw observation facts, socket/session/window/pane selectors, cwd, process identifiers, private persisted objects, or unrelated-resource values.
...
Tmux transport is strict and byte-based in both UTF-8 and non-UTF8 client states. Window creation retains the historical diagnostic grammar `@<digits>|%<digits><LF>`. Current lifecycle observation instead consumes one internal exact-target record: `<socket-path>|<session-id>|<session-name>|<window-id>|<window-name>|<pane-id>|<pane-dead>|<cwd><LF>`. The socket and immutable session/window/pane selectors must match the persisted target. `pane-dead` is exactly `0` or `1`: live state requires `0` and a nonempty exact cwd; dead state requires `1`, an empty current cwd, and the complete persisted cwd. Exactly one terminal LF is required; after the first seven separators any additional vertical bars are cwd bytes. HT, inferred sanitized underscores, alternate separators, missing/extra terminators, invalid selectors/dead flags/cwd, and multiple records are malformed or ambiguous. This exact record, its selectors, `pane_dead`, and cwd remain internal. Public output is categorical, while bounded diagnostics retain only counts and closed value-free structural tokens—never raw output, selector values, cwd/path components, process values, run identities, hashes, or other-run bytes.
...
## Exact tmux context ownership and dead-pane cleanup (0.2.1-beta.2)
...
New `RunSnapshotV6` state persists the canonical socket path and filesystem device/inode plus immutable session, window, pane, and cwd identity. Status, reconcile, resume, stop, clean, attach, and logs always route with the persisted socket selector, regardless of the later caller's tmux context. Attach/capture target the pane ID and cleanup targets the window ID. Complete equality authorizes action only for live lifecycle commands; explicit cleanup refuses a live match and requires the exact dead state described below. Mismatch or unproved absence refuses. A live pane remains required for attach, live logs, resume, stop lineage, and process authority. Explicit `clean` may instead accept one strict dead-pane record only when the socket/session/window/pane selectors still exactly match, `pane_dead` is `1`, current cwd is empty, persisted cwd is complete, and every independent terminal-state, lock, lease, clean worktree branch/HEAD, result, worker, and RPIV fact agrees. Historical process fields and dead state never authorize ownership or mutation. Proved-absent terminal stop/cleanup repeats are stable only after same-owner completion or an exact pre-mutation checkpoint. Raw `TMUX` tuples, server PIDs, malformed values, and unrelated inherited environment values are never persisted or rendered.

Doctor retains its ordered 24 checks. `command.tmux` adds value-free targeting evidence classified as `invoking-valid`, `standalone-fallback`, or `invalid-context` with a closed reason. Before and after classification, Doctor uses explicit `-S` selectors to inventory actual sessions, windows, and panes plus socket filesystem identity on the evidenced custom server and the unrelated default server; fallback or structurally unusable evidence instead compares the explicit default and deterministic standalone selectors. Each query is read-only, bounded to 2 seconds, 1,024 records, and 65,536 bytes. Only value-free unchanged booleans are emitted; socket paths, resource identities, names, and cwd values remain ephemeral. Doctor performs no target mutation or standalone creation. An absent unrelated/default socket is stable empty inventory: Doctor does not invoke tmux for it, does not create or target that server, and does not fail `command.tmux`. An existing stale socket is also stable empty inventory only when its bounded query has zero stdout, exact original-byte `no server running on <queried-socket><LF>` stderr, and unchanged socket type/device/inode. Arbitrary live-server nonzero results, spoofed paths, malformed or additional stderr, timeout, byte/record overflow, EACCES, identity loss, and replacement fail only `command.tmux` with value-free `unavailable-proof` evidence; invalid invoking contexts retain their existing value-free classifications and completed non-tmux observations. Doctor never deletes, creates, or mutates either socket entry.
