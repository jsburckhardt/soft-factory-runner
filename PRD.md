...
The no-retry proof MUST create a private session/dashboard helper, run `has-session`, list the exact dashboard name, prove dashboard helper compound identity and server lineage, create one formatted issue window, strictly parse original `#{window_id}|#{pane_id}<LF>` bytes, set `remain-on-exit`, prove issue helper identity/lineage, perform one strict `#{window_id}|#{pane_id}|#{pane_current_path}<LF>` observation with equal IDs and physical-workspace cwd, and remove that exact window. Each server/client stream MUST count all original bytes and retain at most 4096 bytes; truncation or malformed original bytes fail without retaining values.
...
Tmux identity output is parsed from original bytes in both UTF-8 and non-UTF8 client states. Window creation retains the historical diagnostic grammar `@<digits>|%<digits><LF>`. Current lifecycle observation accepts one internal exact-target record: `<socket-path>|<session-id>|<session-name>|<window-id>|<window-name>|<pane-id>|<pane-dead>|<cwd><LF>`. Canonical socket and immutable session/window/pane selectors must equal the persisted target. `pane-dead` is exactly `0` or `1`; live requires `0` and exact nonempty cwd, while dead requires `1`, empty current cwd, and complete persisted cwd. Exactly one terminal LF is required and only cwd may contain additional vertical bars after the first seven separators. HT, sanitized/inferred forms, alternate separators, extra records or terminators, CR/CRLF, invalid UTF-8, malformed selectors/dead flags/cwd, and partial identifiers are malformed or ambiguous. Exact selector, dead-state, and cwd values stay internal; public status, reconciliation, and control output is categorical and redacted.

Runner may persist only a bounded value-free structural diagnostic: phase, exit code, original stdout/stderr byte counts, at most 8 record and field summaries, and at most 32 tokens distinguishing strict IDs, vertical bars, legacy horizontal tabs, CR/LF, backslashes, and other runs. Raw output, cwd/path components, command/environment/field values, issue/owner/run identities, hashes/byte values, and other-run bytes must never enter that diagnostic.
...
The generic invocation contract is:
