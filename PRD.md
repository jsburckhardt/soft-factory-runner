...
Tmux identity output is parsed from original bytes in both UTF-8 and non-UTF8 client states. Window creation accepts only `@<digits>|%<digits><LF>`; observation accepts only `@<digits>|%<digits>|<cwd><LF>`. Exactly one terminal LF is required. The first two printable vertical bars isolate whole strict IDs and the remaining nonempty valid UTF-8 cwd bytes are retained unchanged, including additional vertical bars. HT, sanitized/inferred forms, alternate separators, extra records or terminators, CR/CRLF, invalid UTF-8 or NUL cwd, empty fields, and partial identifiers are malformed or ambiguous.
