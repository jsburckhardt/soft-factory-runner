...
### Rules
- Persist new runs as `RunSnapshotV6` with the v5 monotonic revision, attempt, lease, launch, process, stop, cleanup, log, merge, final-validation, integration, progress, and nullable latest `TmuxIdentityDiagnosticV1` facts plus complete `TmuxTargetV2` authority. Append `TransitionEventV2` containing prior revision, resulting revision, and the complete redacted resulting `RunSnapshotV3`, `RunSnapshotV4`, `RunSnapshotV5`, or `RunSnapshotV6` before atomic snapshot replacement.
- Read valid snapshot versions 1 through 6 and event versions 1 and 2. Keep v1 readable but non-completable without its missing acceptance input; upgrade eligible v2/v3 records through the existing explicit v4 normalization, normalize supported v4 records through an explicit revisioned v5 transition, and normalize supported exact v4/v5 records only through an explicit revisioned v6 transition that proves complete tmux selectors. Replay only a contiguous, run-identity-matching version 2 chain whose resulting snapshots are valid v3, v4, v5, or v6 records; refuse malformed, conflicting, legacy-ahead, or noncontiguous history.
- Reconcile persisted state separately against issue lock, filesystem, Git branch and worktree including preparation HEAD/cleanliness, exact tmux session/window/pane or name-only presence when no identity is recorded, worker and RPIV process, result artifact, remote branch, and GitHub pull request facts. Classify every observation as `match`, `absent`, `mismatch`, `unknown`, or `not_applicable` and expose the latest bounded tmux identity diagnostic separately. Represent an exact dead-pane observation separately from a live target match: require one complete immutable-selector observation, strict dead flag, empty observed cwd, and complete persisted cwd. Use it only as cleanup-compatible lifecycle corroboration after independent process inactivity and ownership proof; it is not attach, live-log, resume, stop-lineage, or ownership authority.
...
- Make `status`, `list`, `reconcile`, `attach`, `logs`, `resume`, `stop`, `clean`, and existing-state `run` consume the same reconciliation report. Sort `list` by issue number over the union of snapshots, locks, slot leases, and retained logs. Derive human and JSON meaning from the same structured facts. Project command output through explicit public view models rather than serializing snapshots, observations, or reports directly; human and JSON forms must agree while private resource and process values remain unrendered.
...
### Interfaces
- `RunSnapshotV6` carries every v5 ownership, recovery, final-validation, integration-launch, progress, and nullable `tmuxIdentityDiagnostic` fact plus complete `TmuxTargetV2` authority; supported exact v4/v5 input normalizes only through an explicit revisioned transition that proves complete selectors.
- `TransitionEventV2` contains schema version, run and issue identity, prior and resulting revisions, transition reason, timestamp, and a complete resulting `RunSnapshotV3`, `RunSnapshotV4`, `RunSnapshotV5`, or `RunSnapshotV6` for supported history replay.
...
- `ReconciliationReportV3` contains persisted revision, normalized observations for every boundary including non-authorizing progress, the latest bounded tmux identity diagnostic, decision code, active classification, safe actions, and redacted diagnostics. `StatusFactsV5` carries the shared report and control facts for human and JSON rendering.
