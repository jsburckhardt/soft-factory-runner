...
- Observe the complete target once per reconciliation boundary. Return match only for exact socket filesystem, session, window, pane, and cwd equality; return complete absence for a nonzero exact-target query; return unknown for malformed zero-exit output or unavailable proof. Never mix fields from separate observations.
- Allow attach, live logs, resume, stop lineage, or explicit cleanup only from the shared complete-match report. Capture only the exact pane, attach/select only the exact pane, and remove only the immutable window ID on the persisted socket.
