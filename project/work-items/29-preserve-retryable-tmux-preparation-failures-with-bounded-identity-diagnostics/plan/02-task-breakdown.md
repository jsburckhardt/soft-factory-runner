...
### Hosted cleanup portability return evidence for T-9/T-10/T-15

- **Proved product root cause:** Hosted runs `31814521678` and `31816018556` showed two stable complete helper identities before Product Doctor replaced every intended outcome with `helper-stop/process-identity-unknown`. `LiveDoctorProcessPort.findHelpers` was enumerating every `/proc` entry and calling strict ancestry parsing before filtering. Unrelated restricted, malformed, or racing host processes could therefore throw and become cleanup uncertainty even though neither helper identity was mismatched.
- **T-9 correction:** `findHelpers` now validates the exact managed server identity, walks only `/proc/<managed-pid>/task/<managed-pid>/children` recursively, and is bounded to 64 descendants and depth 8. It treats exact ENOENT for a disappearing descendant as absence, but fails safely for an unavailable managed root, malformed tree, malformed owned descendant identity, duplicate/cyclic PID, or bound overflow. Candidate filtering additionally requires the exact physical executable, sole helper argument, workspace cwd, launch interval, and managed-server lineage before retaining the full PID/PGID/start-token identity for exact pre-signal re-observation.
- **T-10 correction:** Cleanup records that a helper may exist before awaiting `new-session` or `new-window`, preserving malformed-create and timeout recovery before pane PID acceptance. Fallback discovery is skipped only before any helper creation command was attempted; private `kill-server` and managed-server cleanup remain unconditional. A malformed-create regression leaves an unrecorded issue helper, proves descendant recovery, and stops it by exact identity.
- **Regression evidence:** `src/doctor-tmux.test.ts` covers unrelated inaccessible/malformed process facts, direct/nested exact helpers, nonmatching descendants, unavailable/malformed owned trees and identities, disappearing descendants, count/depth caps, no-helper scan avoidance with one kill-server, and malformed-create helper recovery. Existing alias acceptance, distinct-executable refusal, exact signal, private socket, one-pass, and cleanup matrices remain passing. The fixture readiness barrier and value-free diagnostics in `src/doctor-integration.test.ts` remain unchanged.
- **T-15 validation:** Targeted process tests passed 1 suite/43 tests; targeted Doctor plus built integration passed 2 suites/52 tests. READY and malformed-create each passed three repeated selected-suite runs. Direct and harness focused gates passed 23 suites/456 tests. Corrected direct and harness full gates passed lint, formatting, strict types, 23 suites/456 tests, build, diff check, and coverage of 88.90% statements, 84.01% branches, 95.42% functions, and 90.50% lines. Controlled workspace/process inventories were empty.
- **Architecture/documentation:** The correction conforms to exact owned-tree cleanup in ADR-260812, CORE-COMPONENT-260812, and decisions 135-143. No ADR, core-component, decision log, application documentation, API, configuration, migration, data, service, container, deployment, or normal issue-run behavior changed.
