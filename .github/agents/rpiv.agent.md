...
You MUST publish bound `rpiv-status.json` progress through the injected Runner helper at every Research, Plan, Implement, Verify, failed, and terminal transition; every failure or exception path MUST attempt terminal `failed` publication before a nonzero return.
...
You MUST exit zero only after that local validator succeeds; preserve the original pipeline error when failed-progress publication also fails, surface both failures observably, and keep Runner authoritative for post-exit Git/GitHub completion reconciliation.
...
TRY:
  ...
  IF PIPELINE_STATUS = "error":
    SET ORIGINAL_FAILURE := VERIFY_RESULT (from "Agent Inference")
    RUN `publish-failed-terminal-progress`
  ...
  IF PIPELINE_STATUS = "error":
    SET ORIGINAL_FAILURE := "Research progress publication failed." (from "Agent Inference")
    RUN `publish-failed-terminal-progress`
    RETURN: format="PIPELINE_ERROR", details=VERIFY_RESULT, error_message="Research progress publication failed", failed_stage="research", issue_number=ISSUE_NUMBER, return_stage="research"
  RUN `dispatch-research`
  IF PIPELINE_STATUS = "error":
    SET ORIGINAL_FAILURE := RESEARCH_RESULT (from "Agent Inference")
    RUN `publish-failed-terminal-progress`
    RETURN: format="PIPELINE_ERROR", details=VERIFY_RESULT, error_message="Research failed", failed_stage=CURRENT_STAGE, issue_number=ISSUE_NUMBER, return_stage="research"
  RUN `publish-plan-progress`
  IF PIPELINE_STATUS = "error":
    SET ORIGINAL_FAILURE := "Plan progress publication failed." (from "Agent Inference")
    RUN `publish-failed-terminal-progress`
    RETURN: format="PIPELINE_ERROR", details=VERIFY_RESULT, error_message="Plan progress publication failed", failed_stage="plan", issue_number=ISSUE_NUMBER, return_stage="plan"
  RUN `dispatch-plan`
  IF PIPELINE_STATUS = "error":
    SET ORIGINAL_FAILURE := PLAN_RESULT (from "Agent Inference")
    RUN `publish-failed-terminal-progress`
    RETURN: format="PIPELINE_ERROR", details=VERIFY_RESULT, error_message="Plan failed", failed_stage=CURRENT_STAGE, issue_number=ISSUE_NUMBER, return_stage="plan"
  RUN `publish-implement-progress`
  IF PIPELINE_STATUS = "error":
    SET ORIGINAL_FAILURE := "Implement progress publication failed." (from "Agent Inference")
    RUN `publish-failed-terminal-progress`
    RETURN: format="PIPELINE_ERROR", details=VERIFY_RESULT, error_message="Implement progress publication failed", failed_stage="implement", issue_number=ISSUE_NUMBER, return_stage="implement"
  RUN `dispatch-implement`
  IF PIPELINE_STATUS = "error":
    SET ORIGINAL_FAILURE := IMPLEMENT_RESULT (from "Agent Inference")
    RUN `publish-failed-terminal-progress`
    RETURN: format="PIPELINE_ERROR", details=VERIFY_RESULT, error_message="Implement failed", failed_stage=CURRENT_STAGE, issue_number=ISSUE_NUMBER, return_stage="implement"
  RUN `publish-verify-progress`
  IF PIPELINE_STATUS = "error":
    SET ORIGINAL_FAILURE := "Verify progress publication failed." (from "Agent Inference")
    RUN `publish-failed-terminal-progress`
    RETURN: format="PIPELINE_ERROR", details=VERIFY_RESULT, error_message="Verify progress publication failed", failed_stage="verify", issue_number=ISSUE_NUMBER, return_stage="verify"
  RUN `dispatch-verify`
  IF PIPELINE_STATUS = "error":
  ...
  IF PIPELINE_STATUS = "error":
    SET ORIGINAL_FAILURE := VERIFY_RESULT (from "Agent Inference")
    RUN `publish-failed-terminal-progress`
    RETURN: format="PIPELINE_ERROR", details=VERIFY_RESULT, error_message="Terminal progress publication failed", failed_stage="verify", issue_number=ISSUE_NUMBER, return_stage="verify"
RECOVER (err):
  SET ORIGINAL_FAILURE := <ORIGINAL_ERROR> (from "Agent Inference" using err; preserve type, message, stage, and handoff evidence)
  RUN `publish-failed-terminal-progress`
  RETURN: format="PIPELINE_ERROR", details=VERIFY_RESULT, error_message="Pipeline exception", failed_stage=CURRENT_STAGE, issue_number=ISSUE_NUMBER, return_stage=FAILURE_OWNER
...
<process id="publish-failed-terminal-progress" name="Publish failed terminal outcome before every nonzero return">
USE `execute/runInTerminal` where: command="<INJECTED_PUBLISH_PROGRESS_COMMAND --phase terminal --status failed>"
CAPTURE FAILED_PROGRESS_RESULT from `execute/runInTerminal`
SET FAILED_PROGRESS_PASSED := <PASSED> (from "Agent Inference" using FAILED_PROGRESS_RESULT; require exit zero and a successful bound publication envelope)
IF FAILED_PROGRESS_PASSED is false:
  SET VERIFY_RESULT := <COMBINED_FAILURE> (from "Agent Inference" using ORIGINAL_FAILURE, FAILED_PROGRESS_RESULT; preserve ORIGINAL_FAILURE verbatim first and append the redacted failed-publication exit, code, and output)
ELSE:
  SET VERIFY_RESULT := ORIGINAL_FAILURE (from "Agent Inference")
SET PIPELINE_STATUS := "error" (from "Agent Inference")
</process>
