...
describe("V3 and V11 packed built-CLI installation smoke", () => {
  ...
  beforeAll(async () => {
    ...
    expect(packEntry.version).toBe("0.2.0");
    expect(packEntry.filename).toBe("soft-factory-runner-0.2.0.tgz");
    ...
    expect(installedMetadata.version).toBe("0.2.0");
  ...
  it("installs individual and recommended forms identically and repeats as a no-op", async () => {
    ...
    for (let index = 0; index < roots.length; index += 1) {
      ...
      expect(generated.assets[0]?.version).toBe("0.2.0");
  ...
  it("reconverges a proved 0.1.0 current manifest to 0.2.0 and repeats", async () => {
    ...
    expect(manifest.assets.map((entry) => entry.version)).toEqual(["0.2.0"]);
