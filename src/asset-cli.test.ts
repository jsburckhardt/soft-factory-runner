...
describe("V3 and V11 packed built-CLI installation smoke", () => {
  ...
  beforeAll(async () => {
    ...
    expect(packEntry.version).toBe("0.2.1-beta.3");
    expect(packEntry.filename).toBe("soft-factory-runner-0.2.1-beta.3.tgz");
    ...
    expect(installedMetadata.version).toBe("0.2.1-beta.3");
  ...
  it("installs individual and recommended forms identically and repeats as a no-op", async () => {
    ...
    for (let index = 0; index < roots.length; index += 1) {
      ...
      expect(generated.assets[0]?.version).toBe("0.2.1-beta.3");
  ...
  it("reconverges a proved 0.1.0 current manifest to 0.2.1-beta.3 and repeats", async () => {
