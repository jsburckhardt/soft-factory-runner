...
Explicit `clean` refuses a live `TMUX_MATCH`; only an exact dead-pane observation plus every independent Core fact can make it eligible. Once eligible, explicit `clean` records intent and progress, captures the transcript, removes the exact terminal tmux window, removes the clean worktree with non-forced `git worktree remove`, releases an exact inactive slot, and compare-deletes the exact issue lock last. It never removes the local branch, snapshot, events, or logs. The final pane transcript is retained before tmux removal. Immediately before each destructive call Runner persists a same owner and run (`same-owner/run`) checkpoint bound to that exact resource identity, then verifies absence and persists completion. That checkpoint is the durable same-owner/run record. A retry accepts absence after either that exact started checkpoint or completed progress, never calls removal twice, and refuses any contradiction or replacement; an absent unrecorded resource or any unrelated replacement blocks. Output identifies completed steps, remaining steps, and remediation.
