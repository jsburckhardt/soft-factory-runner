...
4. create one detached issue window with exact original bytes `#{window_id}<HT>#{pane_id}<LF>` and the strict shared parser;
5. set `remain-on-exit` on that exact window;
6. identify the issue helper with the same compound and lineage proof;
7. make one formatted `list-panes` observation using `#{window_id}<HT>#{pane_id}<HT>#{pane_current_path}<LF>`, strictly parse the original bytes, and require equal IDs plus the physical workspace cwd; and
8. remove the exact issue window.

Creation and observation use horizontal tab as the only separator, LF as the only terminator, and one optional final LF; CR/CRLF, invalid UTF-8 cwd, extra/partial fields, and multiple records fail. Every server/client stdout and stderr stream is drained, counts all original bytes before decoding, and retains at most 4096 bytes. A 4097th byte fails as truncated even though only the cap is retained. No retained or rendered evidence contains a probe value.
