...
Each new `RunSnapshotV4` stores exactly one `requiredFinalValidation`. Runner injects the same immutable command into `IntegrationLaunchV1`; execution, recovery, and completion never reread current configuration for that run. A later new run snapshots the then-current valid value. Supported v1-v3 snapshots deterministically normalize to sole `just verify` and never consult later configuration. A v1 record still cannot complete without a supported migration proving its absent acceptance set. Malformed and unsupported persistence remains non-authorizing.
...
Strict `RpivStatusV1` has exactly `schemaVersion`, `runId`, `attempt`, `issueNumber`, `branch`, `sequence`, `phase`, `status`, and `updatedAt`. Phases advance `research -> plan -> implement -> verify -> terminal`. Nonterminal status is `running`; terminal status is one of `succeeded`, `failed`, `blocked`, `cancelled`, or `interrupted`. Publish each phase start and every failed or terminal outcome.
...
Only after acceptance succeeds, the snapshotted final validation passes, the final head is pushed, and a pull request is created, the Verifier publishes through the injected no-clobber helper to:
...
The command must equal the run snapshot, status must be `passed`, and evidence must be nonempty and redacted. Missing, empty, malformed, required-field-incomplete, unsupported, issue/branch/head/PR/acceptance-mismatched, or final-validation-mismatched results remain noncompleted with safe recovery semantics.
...
Troubleshooting starts with `instructions --json`, then `status --json`. Preserve all ambiguous files. Correct configuration before retrying ownership; republish mutable progress only through its helper; never replace an immutable result; and return nonzero from RPIV when publication or coordinator validation fails.
