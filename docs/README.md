...
Current local package release: **0.2.1-beta.2**, a backward-compatible prerelease PATCH correction for guarded cleanup of an exactly owned remain-on-exit dead pane. Strict dead state is non-authorizing corroboration: cleanup still requires every independent terminal, ownership, lock/lease, clean worktree, result, and process fact. The recovery guide documents transcript retention, exact started checkpoints, idempotent retries, categorical confidential output, isolated proof, and the deferred non-gating Sparkta operator handoff. The exact printable record remains invariant across controlled UTF-8 and non-UTF8 tmux client states. The preceding 0.2.1-beta.1 Doctor correction established that an absent unrelated/default socket does not fail Doctor, is never created or targeted, and arbitrary failures remain value-free `unavailable-proof`. Local package evidence uses no registry publication.
