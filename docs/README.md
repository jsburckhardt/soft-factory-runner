...
Current local package release: **0.2.1-beta.2**, a backward-compatible prerelease PATCH correction only for guarded cleanup of an exactly owned remain-on-exit dead pane. Explicit cleanup refuses a live match; strict dead state is non-authorizing corroboration and cleanup still requires every independent terminal, ownership, lock/lease, clean worktree, result, and process fact. Prerelease history is intentionally separate: 0.2.1-beta.0 prevents Doctor collapse when an unrelated/default tmux server is absent; 0.2.1-beta.1 recognizes only an unchanged stale socket with exact bounded no-server evidence; 0.2.1-beta.2 adds only guarded dead-pane cleanup. The exact printable record remains invariant across controlled UTF-8 and non-UTF8 tmux client states. The recovery guide documents transcript retention, exact started checkpoints, idempotent retries, categorical confidential output, isolated proof, and the deferred non-gating Sparkta operator handoff. Local package evidence uses no registry publication.
