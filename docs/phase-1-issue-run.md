...
Runner parses original command bytes before UTF-8 decoding in explicit UTF-8 and non-UTF8 tmux client states. Window creation keeps the historical diagnostic grammar `@<digits>|%<digits><LF>`, with whole-field `^@[0-9]+$` window and `^%[0-9]+$` pane IDs. Current lifecycle observation consumes one internal exact-target record: `<socket-path>|<session-id>|<session-name>|<window-id>|<window-name>|<pane-id>|<pane-dead>|<cwd><LF>`. Its canonical socket and immutable session/window/pane selectors must equal the persisted target. `pane-dead` accepts only `0` or `1`: a live pane requires `0` and the exact nonempty persisted cwd; a remain-on-exit dead pane requires `1`, empty current cwd, and a complete persisted cwd. Exactly one terminal LF is required. The first seven vertical bars delimit the fixed fields; additional bars belong to cwd. HT, sanitized inference, alternate separators, missing/extra terminators, malformed selectors/dead flags/cwd, and multiple records are malformed or ambiguous.
...
Human and `--json` status derive from the same categorical public view and agree on persisted state, activity, decision/outcome, safe actions, cleanup categories, refusal, remediation, and exit meaning. Neither exposes raw observation facts, socket/session/window/pane selectors, cwd, process identifiers, private persisted objects, or unrelated-resource values. Worker and command success is returned only for `completed`; noncompleted terminal outcomes are nonzero.
